...
void GraphicsEngine::createTextureImage() {
    ...
    createBuffer(imageSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, stagingBuffer_, stagingBufferMemory_);

    void* data;
    vkMapMemory(device_, stagingBufferMemory_, 0, imageSize, 0, &data);
    ...
    copyBufferToImage(stagingBuffer_, textureImage_, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
...
void GraphicsEngine::deviceWaitIdle()
{
    vkDeviceWaitIdle(device_);
}

void GraphicsEngine::frameBufferResized()
{
